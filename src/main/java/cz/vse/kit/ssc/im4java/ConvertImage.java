...
public class ConvertImage extends ImageOperation {
    ...
    public static Screenshot convertToGray(Screenshot screenshot) throws IM4JavaException {
        ...
        try (InputStream inputStreamBaseScreenshot = new ByteArrayInputStream(screenshot.getImageData())) {
    ...
    public static Screenshot compute(Screenshot baseScreenshot, Screenshot otherScreenshot, float baseImageMean,
            float otherImageMean, float commonImageMean) throws IM4JavaException {
        IMOperation op = new IMOperation();
        op.addImage();
        op.addImage();
        op.compose("mathematics");
        op.set("option:compose:args", "1,-" + String.valueOf(baseImageMean) + ",-" + String.valueOf(otherImageMean)
                + "," + String.valueOf(commonImageMean));
        op.composite();
        op.addImage("miff:-");
        ConvertCmd convertCmd = new ConvertCmd();
        logCommand(convertCmd, op);
    ...
    public static float computeCommonMeanAndStandardDeviation(Screenshot screenshot, float firstStandardDeviation,
            float secondStandardDeviation) throws IM4JavaException {
        IMOperation op = new IMOperation();
        // op.addImage("xc:");
        op.addImage("-");
        op.format("%[fx:" + "mean" + "/(" + String.valueOf(firstStandardDeviation) + "*"
                + String.valueOf(secondStandardDeviation) + ")]");
        op.addImage("info:");
        ConvertCmd convertCmd = new ConvertCmd();
        logCommand(convertCmd, op);
